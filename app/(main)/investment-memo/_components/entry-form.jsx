...
import { improveWithAI } from "@/actions/memo";
...
export function EntryForm({ type, entries, onChange }) {
  ...
  useEffect(() => {
    ...
    if (improveError) {
      toast.error(improveError.message || "Failed to enhance description");
    }
      ...
      {isAdding && (
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                <Input type="month" {...register("startDate")} placeholder="eg: 2025-01" />
                {errors.startDate && (
                  <p className="text-sm text-red-500">{errors.startDate.message}</p>
                )}
              </div>
              <div className="space-y-2">
                <Input type="month" {...register("endDate")} disabled={current} placeholder="eg: 2025-12" />
                {errors.endDate && (
